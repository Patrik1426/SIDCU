...
describe("obtenerConfigModuloAutoevaluacion", () => {
  ...
  it("regresa la fila si existe", async () => {
    ...
    const { obtenerConfigModuloAutoevaluacion } = await import("./db");
    const resultado = await obtenerConfigModuloAutoevaluacion();
...
describe("actualizarModuloAutoevaluacionManual", () => {
  beforeEach(() => vi.resetModules());
  ...
  it("actualiza habilitado, limpia la ventana programada, y audita -- todo en una transaccion", async () => {
    ...
    const { actualizarModuloAutoevaluacionManual } = await import("./db");
    await actualizarModuloAutoevaluacionManual(false, 1);
...
describe("programarVentanaModuloAutoevaluacion", () => {
  beforeEach(() => vi.resetModules());
  ...
  it("guarda fechaDesde/fechaHasta y audita en una transaccion", async () => {
    ...
    const { programarVentanaModuloAutoevaluacion } = await import("./db");
    await programarVentanaModuloAutoevaluacion("2026-06-01", "2026-06-30", 1);
