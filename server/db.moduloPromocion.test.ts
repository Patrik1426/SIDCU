...
describe("obtenerConfigModuloPromocion", () => {
  ...
  it("regresa la fila si existe", async () => {
    ...
    const { obtenerConfigModuloPromocion } = await import("./db");
    const resultado = await obtenerConfigModuloPromocion();
...
describe("actualizarModuloPromocionManual", () => {
  beforeEach(() => vi.resetModules());
  ...
  it("actualiza habilitado, limpia la ventana programada, y audita -- todo en una transaccion", async () => {
    ...
    const { actualizarModuloPromocionManual } = await import("./db");
    await actualizarModuloPromocionManual(false, 1);
...
describe("programarVentanaModuloPromocion", () => {
  beforeEach(() => vi.resetModules());
  ...
  it("guarda fechaDesde/fechaHasta y audita en una transaccion", async () => {
    ...
    const { programarVentanaModuloPromocion } = await import("./db");
    await programarVentanaModuloPromocion("2026-06-01", "2026-06-30", 1);
